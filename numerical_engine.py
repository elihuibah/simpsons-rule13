...
def simpson_rule_simple(f, a, b):
    ...
    I_aprox = (b - a) * ((fx0 + (4 * fx1) + fx2) / 6) # Fórmula de Regla de Simpson 1/3 simple
    return I_aprox
...
def simpson_rule_compuesta(f, a, b, n):
    ...
    for i in range(0, n + 1):
        ...
        if(i == 0 or i == n): # Si se tiene xn = x0 o xn (donde n es el valor de intervalos delimitados), al resultado se suma la funcion evaluada en xn
            ...
            I_aprox += 2 * f(xn) # Si i es par, al resultado se suma la funcion evaluada en xn multiplicada por 2

    I_aprox *= (delta_x / 3) # El resultado final es la suma de todo lo anterior multiplicado por delta de x entre 3
    return I_aprox